...
{
	if (canSelect && (Controls.UI_UP_P || Controls.UI_DOWN_P || Controls.MOUSE_WHEEL))
	{
		...
		switch (options[selInt])
		{
			...
			case 'exit to menu':
				if (CoolVars.data.developerMode)
				{
					...
					FlxTween.tween(curSpr, {y: curSpr.y + FlxG.height}, 1, {ease: FlxEase.backIn});
					FlxTween.tween(curSpr, {x: curSpr.x + FlxG.random.float(-300, 300)}, 1, {ease: FlxEase.cubeIn});
